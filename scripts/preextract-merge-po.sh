...
for locale in $LOCALES; do
  ...
  fhir_emr_file="$ROOT_DIR/contrib/fhir-emr/src/locale/$locale/messages.po"
  ...
  existing_files=""
  for candidate in "$app_file" "$fhir_emr_file" "$web_item_controls_file" "$fhir_questionnaire_file"; do
    if [ -f "$candidate" ]; then
